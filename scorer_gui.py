...
from tools import add_transparent_image
...
connecting_overlay = cv2.imread(
    "images/reconnecting.png", cv2.IMREAD_UNCHANGED
)  # IMREAD_UNCHANGED => open image with the alpha channel
button_overlay[:, :, :3] = [0, 0, 255]

if __name__ == "__main__":
    # Create and configure client
    mqtt_client = RobustMQTTClient("DartNet Python Scorer GUI")
    ...
    def score(x, y):
        ...
        print(f"Publishing score {sc}...", end="")
        published = mqtt_client.publish(topic, sc, qos=1)
        print(f'{"OK" if published else "Error"}')
    ...
    while True:
        ...
        if not connected:
            tmp = img.copy()
            offx = 150 - connecting_overlay.shape[1] // 2
            offy = 150 - connecting_overlay.shape[0] // 2
            add_transparent_image(tmp, connecting_overlay, offx, offy)
            cv2.imshow("Scorer", tmp)
        else:
            cv2.imshow("Scorer", img)
