...
def refine_sub_pix(pts, img):
    ...
    tr_xy = [
        [c for c in p]
        for p in pts
        if (p[1] >= 0 and p[1] < img_gray.shape[0]) and (p[0] >= 0 and p[0] < img_gray.shape[1])
    ]
...
class YoloTargetDetector:
    ...
    def detect(self, img, refine_pts=True, dbg=None):
        ...
        if dbg is not None:
            for i in fd:
                cv2.drawMarker(
                    dbg, corners[i].astype(np.int32), (0, 100, 255), cv2.MARKER_TILTED_CROSS, 10, 1, cv2.LINE_AA
                )
        ...
        # -------------
        M = ransac_fit(
            PerspectiveBoardFit(self.pts, corners),
            np.array(closest_point_pairs, np.int32),
            success_probabilities=0.99,
            outliers_ratio=0.6,
            inliers_thres=5,
        )
        if M is None:
        ...
        for i, d in enumerate(distances):
            if d < 10:
                ...
                if dbg is not None:
                    cv2.line(
                        dbg,
                        projected[i].astype(np.int32),
                        corners[indices[i]][0].astype(np.int32),
                        (0, 0, 255),
                        2,
                        cv2.LINE_AA,
                    )
        ...
        if self.auto_am_calib:
            cameraMatrixInit = np.array(
                [[2000.0, 0.0, img.shape[1] / 2.0], [0.0, 2000.0, img.shape[0] / 2.0], [0.0, 0.0, 1.0]]
            )
            ...
            ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(
                np.array([model]).astype(np.float32),
                np.array([corners[valid_pairs[:, 1]]]).astype(np.float32),
            ...
            undist = cv2.undistortImagePoints(
                np.array([transform_points(self.board.board_cal_pts, M)]).astype(np.float32), mtx, dist
            )
    ...
    def draw_board(self, img):
        detected_center = (
            (np.array([self.bouter[0], self.bouter[1]]) + np.array([self.bouter[2], self.bouter[3]])) * 0.5
            if self.bouter is not None
            else None
        )
        ...
        if self.bouter is not None:
            cv2.rectangle(
                img,
                (int(self.bouter[0]), int(self.bouter[1])),
                (int(self.bouter[2]), int(self.bouter[3])),
                (0, 255, 0),
                1,
                cv2.LINE_AA,
            )
        if self.binner is not None:
            cv2.rectangle(
                img,
                (int(self.binner[0]), int(self.binner[1])),
                (int(self.binner[2]), int(self.binner[3])),
                (0, 0, 255),
                1,
                cv2.LINE_AA,
            )
    ...
    def get_dart_scores(self, tips_pts, numeric=False):
        ...
        if self.bouter is not None:
            detected_center = (
                np.array([self.bouter[0], self.bouter[1]]) + np.array([self.bouter[2], self.bouter[3]])
            ) * 0.5
        elif self.binner is not None:
            detected_center = (
                np.array([self.binner[0], self.binner[1]]) + np.array([self.binner[2], self.binner[3]])
            ) * 0.5

        M = cv2.getPerspectiveTransform(
            np.array(self.pts_cal).astype(np.float32), np.array(self.board.board_cal_pts).astype(np.float32)
        )
    ...
    for path in tests:
        ...
        def get_points_1(img):
            ...
            detected_circles = cv2.HoughCircles(
                gray_blurred, cv2.HOUGH_GRADIENT, 1, 20, param1=50, param2=30, minRadius=600, maxRadius=3000
            )
            ...
            if detected_circles is not None:
                ...
                for pt in detected_circles[0, :]:
                    ...
                    if abs(a - W * 0.5) > W * 0.2:
                        continue
                    if abs(b - H * 0.5) > H * 0.2:
                        continue
        ...
        def get_points_3(img):
            ...
            corners = cv2.goodFeaturesToTrack(gray_image, maxCorners=200, qualityLevel=0.005, minDistance=20)
        ...
        def get_points_4(img, pyr=False):
            ...
            if pyr:
                ...
                res, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
            shifted = im_src.copy()
            ...
            def find_center_from_sectors_lines(edged, dbg):
                ...
                for a in filtered_angles:
                    ...
                    cv2.line(
                        dbg,
                        (int(center[0] + dx * 2), int(center[1] + dy * 2)),
                        (int(center[0] - dx * 2), int(center[1] - dy * 2)),
                        col,
                        1,
                        cv2.LINE_AA,
                    ...
                    cv2.putText(
                        dbg, f"{a:.2f}", (int(center[0] + dx), int(center[1] + dy)), 0, 0.5, color=col, thickness=2
                    )
            ...
            def sobel_threshold(img, ksize=5, thres=200):
                ...
                kernel = np.array(
                    [[0.05, 0.25, 0.05], [0.25, 0.5, 0.25], [0.05, 0.25, 0.05]]
            ...
            def ellipse_from_watershed(im_src, img, mean_radius=160):
                ...
                for label in cpt.keys():
                    ...
                    contours, hierarchy = cv2.findContours(
                        ((markers == label) * 255).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
                    )
                    for cnt in contours:
                        epsilon = 0.008 * cv2.arcLength(cnt, True)
                        approximations = cv2.approxPolyDP(cnt, epsilon, True)
                        if (
                            np.sum(approximations[:, :, 0] < 10) == 0
                            and np.sum(approximations[:, :, 0] > markers.shape[1] - 10) == 0
                            and np.sum(approximations[:, :, 1] < 10) == 0
                            and np.sum(approximations[:, :, 1] > markers.shape[0] - 10) == 0
                        ):
                            cv2.drawContours(
                                dbg,
                                [approximations],
                ...
                el = FitEllipse_RANSAC(
                    nzero,
                    tmp_ellipse.astype(np.float32),
                    graphics=False,
                    success_probabilities=0.99,
                    outliers_ratio=0.5,
                    inliers_dist=1.5,
                ...
                if el is not None:
                    ...
                    cv2.ellipse(
                        img,
                        center=np.array(el[0]).astype(np.int32),
                        axes=(int(el[1][0] * 0.5), int(el[1][1] * 0.5)),
                        angle=el[2],
                        startAngle=0,
                        endAngle=360,
                        color=(0, 255, 255),
            ...
            def ellipse_detector_test_from_sat(center, angular_resol=200, dis_max=400):
                ...
                for i, a in enumerate(np.arange(0, 360, 360 / angular_resol)):
                    ...
                    ptsl, dist = segment_intersection_points(
                        np.array(center, dtype=np.int32),
                        np.array([center[0] + dx, center[1] + dy], dtype=np.int32),
                        s,
                        THRES=128,
                    )
                ...
                def get_ellipse_perspective_tr(e, center, sz=512, compute_inverse=False, dbg=False, **kwargs):
                    ...
                    if dbg:
                        if axes[0] > 0.0001 and axes[1] > 0.0001:
                            cv2.ellipse(
                                img,
                                center=c.astype(np.int32),
                                axes=tuple(axes.astype(np.int32)),
                                angle=el[2],
                                startAngle=0,
                                endAngle=360,
                                **kwargs,
                    ...
                    if dbg:
                        ...
                        cv2.line(
                            img,
                            np.array(ptis1[0]).astype(np.int32),
                            np.array(ptis1[1]).astype(np.int32),
                            color=(255, 0, 255),
                        )
                        for pti in ptis2:
                        ...
                        cv2.line(
                            img,
                            np.array(ptis2[0]).astype(np.int32),
                            np.array(ptis2[1]).astype(np.int32),
                            color=(50, 128, 255),
                        )
                ...
                el = ransac_fit(
                    EllipseModel(), extern_pts, success_probabilities=0.99, outliers_ratio=0.5, inliers_thres=5
                )
                if el is None:
                    return None
                ...
                radius = ransac_fit(
                    FixedCenterCircleBoardModel(center_u, board, min_dist=50),
                    pts_u,
                    success_probabilities=0.995,
                    outliers_ratio=0.6,
                    inliers_thres=5,
                )
        ...
        def viola_jones_test(img):
            ...
            # Apply Hough transform on the blurred image.
            detected_circles = cv2.HoughCircles(
                gray_blurred, cv2.HOUGH_GRADIENT, 1, gray.shape[0] / 8, param1=200, param2=100, minRadius=0, maxRadius=0
            )
            ...
            def filter_lines(lines, sz, radius=3):
                ...
                for i, line in enumerate(lines):
                    l = line[0]
                    if (l[0] >= 0 and l[0] < sz[1] and l[1] >= 0 and l[1] < sz[0] and tmp[l[1], l[0]] == 0) or (
                        l[2] >= 0 and l[3] < sz[1] and l[2] >= 0 and l[3] < sz[0] and tmp[l[3], l[2]] == 0
                    ):
            ...
            for bdi, bd in enumerate(boards):
                ...
                if lines is not None:
                    ...
                    for li, line in enumerate(lines):
                        l = line[0]
                        if (l[0] >= bd[0] and l[0] < bxmax and l[1] >= bd[1] and l[1] < bymax) or (
                            l[2] >= bd[0] and l[2] < bymax and l[3] >= bd[1] and l[3] < bymax
                        ):
                            ids_in.append(li)
