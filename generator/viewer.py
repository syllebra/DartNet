...
class Application(SimpleViewer):
    def __init__(self, master=None, img=None, menu=True, show_straighten=True):
        ...
        if show_straighten:
            ...
            # -----  NEW  -----
            # when the window is closed, forget the viewer
            second_win.bind("<Destroy>", lambda e: setattr(self, "straighten_viewer", None))
        else:
            self.straighten_viewer = None
        ...
        self.metadata = {
            "kc": {"cal1": [0, 0], "cal2": [0, 0], "cal3": [0, 0], "cal4": [0, 0]},
            "board": {
        ...
        self.popup_menu.add_command(label="Add dart marker", command=self.add_dart_marker)
    ...
    def create_menu(self):
        ...
        self.file_menu.add_command(
            label="Save Straighten Image", command=self.menu_save_straighten_clicked, accelerator="Ctrl+E"
        )
    ...
    def reload_metadata(self, event=None):
        ...
        if os.path.exists(metadata_filepath):
            ...
            self.metadata = {
                "kc": {
                    "cal1": [mx, self.orig_pil_image.height * 0.1],
                    "cal2": [mx, self.orig_pil_image.height * 0.9],
                    "cal3": [self.orig_pil_image.width * 0.1, my],
                    "cal4": [self.orig_pil_image.width * 0.9, my],
                }
            }
    ...
    def draw_image(self, pil_image):
        ...
        if self.straighten_viewer is not None:
            straighten = self.straighten_image(
                self.orig_pil_image, draw_circles=self._drag_data["item"] in [None, "test"]
            )
        ...
        if "bbox" in self.metadata:
            for k, box in self.metadata["bbox"].items():
                bx = [[box[0][0], box[0][1]], [box[0][0], box[1][1]], [box[1][0], box[1][1]], [box[1][0], box[0][1]]]
