...
def extend(box, pts, offset=[0, 0, 0], dim=3):
    ...
    if isinstance(pts, dict):
        pts = [[pts["min"][0], pts["min"][1], pts["min"][2]], [pts["max"][0], pts["max"][1], pts["max"][2]]]
    for p in pts:
        for i in range(dim):
...
def make_dart_def(
    ...
    tip_length=0.036,
    barrel="01",
    barrel_mat="tungstene",
    grip_normal_path=None,
    shaft="78262",
    shaft_mat="smooth_plastic",
    shaft_color=[0, 0, 0],
    flight="standard",
    flight_in=0.01,
    flight_texture_file="3D/Flights/country-flags/png100px/sh.jpg",
    **params,
):

    def _obj_def(file, tr=None, mat_ref="smooth_plastic"):
        ret = {"type": "obj", "filename": file, "face_normals": False, "material": {"type": "ref", "id": mat_ref}}
        if tr is not None:
            ret["to_world"] = tr
        return ret
    ...
    dart_def["barrel"] = _obj_def(
        f"3D/Darts/_gen/BARRELS/{barrel}.obj", tr=mi.ScalarTransform4f.translate([0.0, y, 0]), mat_ref=barrel_mat
    )
    box = extend(box, darts_metadata[f"{barrel}.obj"], [0, y, 0])
    y += darts_metadata[f"{barrel}.obj"]["max"][1]
    dart_def["shaft"] = _obj_def(
        f"3D/Darts/_gen/SHAFTS/{shaft}.obj", tr=mi.ScalarTransform4f.translate([0.0, y, 0]), mat_ref=shaft_mat
    )
    y += darts_metadata[f"{shaft}.obj"]["max"][1] - flight_in
    box = extend(box, darts_metadata[f"{shaft}.obj"], [0, y, 0])
    dart_def["flightA"] = _obj_def(
        f"3D/Darts/_gen/FLIGHTS/{flight}.obj", tr=mi.ScalarTransform4f.translate([0.0, y, 0]), mat_ref="flights_mat"
    )
    dart_def["flightB"] = _obj_def(
        f"3D/Darts/_gen/FLIGHTS/{flight}.obj",
        tr=mi.ScalarTransform4f.rotate([0, 1, 0], 90).translate([0.0, y, 0]),
        mat_ref="flights_mat",
    )
    flbx = [
    ...
    if grip_normal_path is not None:
        dart_def["barrel"]["material"] = {
            "type": "normalmap",
            "normalmap": {"type": "bitmap", "raw": True, "filename": grip_normal_path},
        }
...
def make_board_def(board_image_path="3D/Boards/unicorn-striker.jpg", board_thickness=0.0, **params):
    ...
    board_def = {
        ...
        "board_face": {
            ...
            "face_normals": False,
            "to_world": mi.ScalarTransform4f.translate([0, 0, board_thickness])
            .rotate([1, 0, 0], angle=-90)
            .scale(board_radius + 0.002),
            "material": {
                "type": "principled",
                ...
                #'clearcoat_gloss': 0.3,
                # 'spec_trans': 0.4,
                "flatness": 0.15,  # ,'wrap_mode': 'mirror'}
            },
        },
    }
...
def render(render_size=800, spp=35, **params):
    ...
    scene_base = {
        ...
        "wall": {"type": "rectangle", "material": {"type": "diffuse"}},
    }
    ...
    def add_dart(dic, name, x=0.0, y=0.0, x_angle=0.0, y_angle=0.0, roll_angle=0.0, penetration=0.013, ref="dart"):
        transform = (
            mi.ScalarTransform4f.translate([x, y, board_thickness])
            .rotate([1, 0, 0], angle=90 - x_angle)
            .rotate([0, 0, 1], angle=y_angle)
            .rotate([0, 1, 0], angle=roll_angle)
            .translate([0, -penetration, 0])
        )
        dic[name] = {"type": "instance", "to_world": transform, "shapegroup": {"type": "ref", "id": ref}}
    ...
    def backproject(pts, sensor):
        ...
        film = sensor.film()
        prj = mi.perspective_projection(
            film.size(), film.crop_size(), film.crop_offset(), fov, sensor.near_clip(), sensor.far_clip()
        )
        projs = []
        for p in pts:
            if p is not None:
    ...
    def backproject_uvs(pts_uvs, sensor):
        ...
        to_backproj = []
        for p in pts_uvs:
            inter = bf.eval_parameterization(
                uv=mi.Point2f(
                    p[0] / (board_metadata["board"]["width"] - 1), p[1] / (board_metadata["board"]["height"] - 1)
                ),
    ...
    sensors = [
        load_sensor(render_size, sr[0], sr[1], board_metadata["board"]["r_board"], board_thickness, fov, factors[i])
        for i, sr in enumerate(sensors_rot)
    ]

    projs_cal = [
        backproject_uvs([p for k, p in board_metadata["kc"].items() if k in ["cal1", "cal2", "cal3", "cal4"]], sensor=s)
        for s in sensors
    ]
    ...
    projs_darts_bbox = [
        backproject_darts_bbox(dart_bbox, [d["to_world"] for d in darts.values()], sensor=s) for s in sensors
    ]
...
def render_and_save(out_dir="_GENERATED", render_size=800, spp=35, **params):
    images, projs_cals, projs_darts, projs_darts_bbox, board_metadata = render(
        render_size=render_size, spp=spp, **params
    )
    ...
    for ii, image in enumerate(images):
        ...
        mi.Bitmap(image).convert(
            pixel_format=mi.Bitmap.PixelFormat.RGB, component_format=mi.Struct.Type.UInt8, srgb_gamma=True
        ).write(img_save_path)

        save_metadata(
            f"{os.path.splitext(img_save_path)[0]}.json",
            board_metadata,
            projs_cals[ii],
            projs_darts[ii],
            projs_darts_bbox[ii],
...
if __name__ == "__main__":
    ...
    params["dart_def"] = {
        ...
        "shaft": "78262",
        "shaft_mat": "smooth_plastic",
        "shaft_color": [1, 1.0, 1.0],
        "flight": "standard",
        "flight_in": 0.01,
        "flight_texture_file": "3D/Flights/arts/ComfyUI_temp_pmzjp_00095_.jpg",
    }
